...
The iOS runtime ships as a single **`BridgeKit.xcframework`** binary. All Swift modules
(`BridgeKit`, `BridgeKitContracts`, `BridgeKit`, and others) are statically linked in. Consumers
import `BridgeKit` and `BridgeKitContracts` via synthetic thin framework aliases whose
...
xcframework is what app targets link against. See [Installation](/start/installation/) for
setup instructions.
