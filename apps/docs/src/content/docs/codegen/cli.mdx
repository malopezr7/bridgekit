...
| `--package <pkg>` | derived from id | Kotlin package override (default `com.bridgekit.contracts.<id>`). Ignored by the Swift emitter. |
...
- Shared object types (`struct` with the compiler's memberwise `init`)
- `protocol <ClassName>: AnyObject` — the provider protocol
- `protocol <ClassName>Client: AnyObject` — the consumer protocol
...
Declarations carry no explicit access modifier, so they are `internal` — generated files are
compiled as part of the target that owns them, not shipped as a separate module. See [Anatomy
of generated Swift](/codegen/generated-swift/).
...
serializable descriptor (`$type` tag, duck-typed, never `instanceof`).
